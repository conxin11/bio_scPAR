...
class RegulationNetwork:
    ...
    def create_network(self,return_df=True):
        ...
        self.data = data
        self.data_full_columns = interactions[['TF', 'Target', 'Up_or_Down_or_Unknown']]
        self.data_full_columns = self.data_full_columns.copy()
        self.data_full_columns['Type'] = 'Gene'
        self.data_full_columns.loc[self.data_full_columns['TF'].isin(tf), 'Type'] = 'TF'
        if return_df:
            return data
...
--> 表示up, --| 表示down, -/- 表示unknown
self.data_full_columns：
