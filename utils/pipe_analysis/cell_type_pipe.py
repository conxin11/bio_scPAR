...
from utils.pipe_analysis.interaction_type_ar_metrics_statistics import NetworkIntegration, Statistics
class CellTypePipe:
    ...
    def subset_by_genes(self, adata_subs, bionet_fp_dict, deg_sel=True, network_sel=True):
        for cell_type, adata in adata_subs.items():
            ...
            if deg_sel:
                ...
                #enrichment analysis for seed genes by reactome
                deg_reactome_gesa_fp =  os.path.join(Config.gsea_fp, cell_type + "_" + "gsea_reactome_deg.tsv")
                gsea_results = self.gesa(gene_list=seed_genes,
                                            gsea_threshold_dict=Config.gsea_threshold_dict,
                                            gsea_meta=Config.gesea_meta,
                                            gmt_type="reactome_gmt_fp",
                                            save_fp=deg_reactome_gesa_fp)
                #enrichment analysis for seed genes by go
                deg_go_gesa_fp = os.path.join(Config.gsea_fp, cell_type + "_" + "gsea_go_deg.tsv")
                gsea_results = self.gesa(gene_list=seed_genes,
                                            gsea_threshold_dict=Config.gsea_threshold_dict,
                                            gsea_meta=Config.gesea_meta,
                                            gmt_type="go_gmt_fp",
                                            save_fp=deg_go_gesa_fp)
    ...
    def graph_and_gsea_steps(self, results, meta_dict):
        ...
        for cell_type, gene_list in gene_dict.items():
            ...
            save_fp = os.path.join(meta_dict["gsea_outdir"], cell_type + "_reactome_enrich.tsv")
            ...
            gsea_results = self.gesa(gene_list=gene_list,
                                     ...
                                     gmt_type="reactome_gmt_fp",
                                     save_fp=save_fp)
            save_fp = os.path.join(meta_dict["gsea_outdir"], cell_type + "_go_enrich.tsv")
            gsea_results = self.gesa(gene_list=gene_list,
                                        gsea_threshold_dict=meta_dict["gsea_threshold_dict"],
                                        gsea_meta=meta_dict["gsea_meta"],
                                        gmt_type="go_gmt_fp",
